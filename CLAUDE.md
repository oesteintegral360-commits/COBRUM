...
- **Fase 5 (hecha): cobros y conciliación.** Registrar cobro por método (`cobranzas/cobros.py`, modelo `Cobro`): **efectivo** → estado `a_rendir` → `rendido` (la marca el vendedor; pantalla **Caja** `/cobros` muestra la "plata en la calle" por vendedor); **transferencia** → `a_conciliar` → `conciliado` cruzando contra el extracto bancario subido (match por monto). Al registrar un cobro, si salda la deuda vencida el cliente pasa a `estado_gestion='pagado'` y sale de la cobranza; si es **parcial (a cuenta)** queda `activo` y sigue en la agenda por el resto. **Anti-doble-conteo:** solo los cobros registrados DESPUÉS de la última foto descuentan la deuda mostrada (`cobros.cobros_desde_ultima_foto`); los anteriores ya están reflejados en el saldo de esa foto, así un pago nunca se cuenta dos veces.
